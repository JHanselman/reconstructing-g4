...
# for compiling acb_theta_cli on machines where you can't install packages
# or where the system FLINT is older than 3.1 (e.g. Ubuntu 24.04's 3.0.1).
#
#   PREFIX=$HOME/.local/acb_theta sh build_deps.sh     (default shown)
...
#
# Before running this, check whether FLINT >= 3.1 is already available:
#   ls /usr/include/flint/acb_theta.h  ||  module avail 2>&1 | grep -i flint

set -e

PREFIX=${PREFIX:-$HOME/.local/acb_theta}
JOBS=${JOBS:-4}
